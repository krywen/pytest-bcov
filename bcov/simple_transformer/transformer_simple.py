...
from ast import Name, Compare, BoolOp


class SimpleBooleanTransformer(ast.NodeTransformer):
    def __init__(self, filename: str):
        # Store the filename when we create the transformer
        self.filename = filename
        print("SimpleBooleanTransformer Started")
        super().__init__()
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> ast.AST:
        ...
        for child in node.values:
            ...
            if isinstance(child, Compare):
                ...
                pass
            elif isinstance(child, BoolOp):
                print(f"BoolOp: op {child.op}")
            else :
                print(f"type unsure: name {child.id}")
